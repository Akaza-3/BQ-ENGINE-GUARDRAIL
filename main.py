...
def build_ui_report(branch: str, commit_message: str, changed_results: list, beam_filenames: list) -> dict:
    queries = []
    all_business_logic = []
    ...
    for r in changed_results:
        ...
        queries.append({
            ...
            "original_tb_scanned": round((r["new_bytes"] or 0) / 1e12, 4),
            "original_cost_per_run": round(original_cost_per_run, 6),
            "optimized_tb_scanned": round((r["rewrite_bytes"] or 0) / 1e12, 4),
...
def review():
    ...
    ui_report = build_ui_report(branch, commit_message, changed_results, beam_filenames)
