...
# beam_consumer maps each query to the file that reads it.
# -----------------------------------------------------------------
QUERY_RUNTIME_CONFIG = {
...
def ask_gemini_for_rewrite(old_sql, new_sql, cache_name, schema_manifest,
                           beam_context, original_bytes, consumer_file="") -> str:
    prompt = f"""You are an expert Google BigQuery SQL optimizer.
...
# =================================================================
def analyze_one(change: dict, beam_context: str):
    """Dry run -> Gemini rewrite -> dry run -> risk detection for one
    SQL file. Returns (result_dict, cache_info)."""
    ...
    rewrite = ask_gemini_for_rewrite(
        ...
        schema_manifest, beam_context, new_bytes, consumer_file=consumer,
    )
...
def review():
    ...
    for change in changed:
        ...
        r, cache_info = analyze_one(change, beam_context)
...
def analyze():
    ...
    ui_report = build_ui_report(
        ref, f"Full repository scan · {head_sha}",
